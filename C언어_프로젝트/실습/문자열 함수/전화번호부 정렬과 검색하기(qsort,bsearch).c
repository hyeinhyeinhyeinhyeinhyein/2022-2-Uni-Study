...
void main() {
	...
	// 원하는 사람의 이름을 입력받고, 그 사람의 이름이 있는 구조체의 주소를 반환한다. 
	printf("전체 정보가 궁금한 사람의 이름을 작성해주세요: ");
	scanf("%s", whatfind);
	ptr = bsearch(whatfind, hyeinbook, NUMBER_OF_BOOK, sizeof(hyeinbook[NUMBER_OF_BOOK]), namecompare);

	if (ptr == NULL) printf("존재하지 않는 이름입니다.");
	else {
		printf("이름이 존재합니다.\n");

		// 이름이 존재하는 구조체의 주소에서 구조체의 첫 시작 주소를 빼면 내가 찾는 구조체 배열이 구조체의 시작으로부터 얼마만큼 멀어져있는지 알 수 있다. 
		int distance = ptr - hyeinbook;

		// 그 거리를 구조체 배열 1개의 크기로 나누면 그 거리에 몇 개의 구조체가 들어갈 수 있는지 파악할 수 있고, 그 개수가 곧 몇 번째 배열인지, 인덱스가 된다. 
		printf("이름: %s ", hyeinbook[distance / sizeof(hyeinbook[0])].bookname);
		printf("전화번호: %s ", hyeinbook[distance / sizeof(hyeinbook[0])].bookphone);
		printf("주소: %s ", hyeinbook[distance / sizeof(hyeinbook[0])].bookaddress);
	}
}
